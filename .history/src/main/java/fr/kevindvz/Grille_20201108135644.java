...
public class Grille {
    ...
    // rappel de la formultion : variableAChanger = Comparé > Comparé ? resultatAttribuéSivrai : ResultatAttribuésiFaux;
    this.etatGrille = map.get(1) == map.get(2) ? map.get(2) == map.get(3) ? "Ligne gagnante" :  map.get(1) == map.get(4) ? map.get(4) == map.get(7) ? "Colonne gauche" : map.get(1) == map.get(5) ? map.get(5) == map.get(9) ? "diagonale gagnante" : map.get(3) == map.get(6) ? map.get(6) == map.get(9) ? "Ligne droite gagnante" : 
